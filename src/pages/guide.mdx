...
The `fieldName` option is important because we'll be making GraphQL queries against this exact name in the next step. We also include `gatsby-plugin-react-helmet` so that the HTML title and meta tags set by `react-helmet` (also in the next step) get statically rendered in production builds.
